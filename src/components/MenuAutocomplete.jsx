...
export default class MenuAutocomplete extends Component {
    ...
    onChange(event) {
        ...
        if (substring.length > 0) {
            this.setState((prevState) => ({
                ...prevState,
                possibleMatches: this.state.materialsList.filter((material) =>
                    material.includes(substring.toLowerCase())
                ),
            }))
        } else {
            this.setState((prevState) => ({
    ...
    render() {
            ...
            matches = (
                <div className="p-4 pt-0 max-h-56 overflow-y-auto">
                    {this.state.possibleMatches.map((match) => (
                        <p
                            className="font-light py-2 capitalize"
                            key={match}
                            onClick={this.selectMaterial.bind(this)}
                        >
                            {match}
                        </p>
                    ))}
        ...
        return (
            <>
                <input
                    className="w-full p-4 rounded-2xl font-bold bg-brown placeholder-brown-600 text-brown-600 focus:outline-none"
                    ref={this.inputObject}
                    type="text"
